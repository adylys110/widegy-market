...
import {
  ...
  AlertCircle, Building2, CreditCard, Loader2, ChevronDown, Info
} from 'lucide-react'
...
function WithdrawalModal({ balance, bankName, bankAccountName, bankAccountNo, onClose, onSuccess }: {
  ...
  const handleSubmit = async () => {
    ...
    if (!amt || amt < MIN_WITHDRAW) {
      toast.error(`Minimum penarikan ${formatCurrency(MIN_WITHDRAW)}`)
      return
    }
    if (amt > balance) {
    ...
    >
      ...
      >
        <div className="p-6 border-b border-border">
          <h2 className="font-display font-extrabold text-foreground text-lg">Tarik Saldo</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex items-start gap-2 p-3 bg-blue-50 rounded-xl border border-blue-100">
            ...
            <p className="text-xs text-blue-700">Proses transfer 1-3 hari kerja. Pastikan data rekening sudah benar.</p>
          </div>
        </div>
...
export function SellerWithdrawalsClient({ data }: { data: any }) {
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6 max-w-4xl">
      ...
      <motion.div variants={itemVariants} className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-display font-extrabold text-foreground">Penarikan Dana</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Kelola penarikan saldo dari penjualan</p>
        </div>
        <button
          onClick={() => setShowModal(true)}
          disabled={balance < MIN_WITHDRAW}
          className="flex items-center gap-2 px-4 py-2.5 bg-primary text-white rounded-xl text-sm font-bold hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-glow-primary"
        >
        ...
        <div className="divide-y divide-border">
          {withdrawals.length === 0 ? (
            <div className="flex flex-col items-center py-12 text-center">
              ...
              <p className="text-xs text-muted-foreground">Klik "Tarik Dana" untuk membuat pengajuan</p>
            </div>
            ...
            return (
              <div key={w.id} className="flex items-center gap-4 px-6 py-4 hover:bg-muted/20 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {w.adminNote && (
                    <p className="text-xs text-destructive mt-0.5">{w.adminNote}</p>
                  )}
                </div>
