...
import { AffiliatorLayout } from '@/components/affiliator/AffiliatorLayout'
import { BannedScreen } from '@/components/shared/BannedScreen'

export default async function AffiliatorRootLayout({ children }: { children: React.ReactNode }) {
  const session = await auth()

  if (!session?.user) redirect('/login')
  // SELLER juga boleh (untuk yang punya affiliatorProfile)
  ...
  return <AffiliatorLayout>{children}</AffiliatorLayout>
}
